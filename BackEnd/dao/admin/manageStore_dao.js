module.exports = function () {
  ...
  this.totalAllStoreDao = (data) => {
    return new Promise(async function (resolve) {
      ...
      db('store')
        .select('id', 'storeID', 'email', 'dueDay', 'mobileNumber', 'storeName', 'storeImage', 'storeAddress', 'managerFname', 'latitude', 'longitude', 'storeRadius', 'managerLname', 'status', 'isComingSoon', db.raw('DATE_FORMAT(createdAt, "%d/%m/%Y") AS createdDate'), db.raw('TIME_FORMAT(createdAt, "%r") AS time'))
        .where('isStoreDelete', 0)
        .orderBy('id', 'desc')
