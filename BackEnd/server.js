...
async function auth (request, response, next) {
  ...
  try {
    ...
    if (auth.error == 'true') {
      ...
      if(request.headers.role == 'storemanager'){
        request.params.auth.storeId = authenticate.data[0].storeId
      } else if(request.headers.role == 'driver') {
        request.params.auth.carId = authenticate.data[0].carId
      }
...
async function setting (request, response, next) {
  var setting = await this.getStoreDistanceDao(request.body)
  if (setting.error == 'true') {
    error.error = 'true'
    error.message = 'UNAUTHORIZED'
    return response.status(401).send(error)
  } else {
    request.params.setting = setting.result
    next()
  }
}
...
// require('./routes/doctor_account_routes.js')(app, validator)
require('./routes/user_routes.js')(app, validator)
require('./routes/store_routes.js')(app, validator)
require('./routes/storemanager_routes/storeManager_routes.js')(app, validator)
require('./routes/storemanager_routes/order_routes.js')(app, validator)
require('./routes/driverRoutes.js')(app, validator)
